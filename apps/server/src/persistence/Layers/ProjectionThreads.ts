...
const makeProjectionThreadRepository = Effect.gen(function* () {
  ...
  `;
  const hasModelSelectionColumn = threadColumns.some((column) => column.name === "model_selection_json");
  const hasLegacyModelColumn = threadColumns.some((column) => column.name === "model");

  const upsertProjectionThreadRow = SqlSchema.void({
    Request: ProjectionThread,
    execute: (row) =>
      hasModelSelectionColumn
        ? hasLegacyModelColumn
          ? sql`
