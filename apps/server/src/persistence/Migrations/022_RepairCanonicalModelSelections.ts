...
export default Effect.gen(function* () {
  ...
  `;
  const hasProjectDefaultModelSelection = projectColumns.some(
    (column) => column.name === "default_model_selection_json",
  );
  const hasProjectLegacyDefaultModel = projectColumns.some((column) => column.name === "default_model");
  ...
  if (hasProjectLegacyDefaultModel) {
    yield* sql`
  ...
  `;
  const hasThreadModelSelection = threadColumns.some(
    (column) => column.name === "model_selection_json",
  );
  const hasThreadLegacyModel = threadColumns.some((column) => column.name === "model");
